...
3. **Stage 3 — 视觉 [[Propositional Planning|命题]] 分类器**: 用 [[ConvNeXt]]-Tiny 学一个 image → $m$ 维 0/1 向量的映射，测试时在线把任意 RGB 图二值化成谓词，喂给经典 [[A-star Search|A\*]]/BFS 规划器
...
- **测试时**: 0.5 阈值二值化 → 喂给经典 [[A-star Search|A\*]]/BFS 规划器 → 输出动作序列
...
- **测试时规划器**: BFS / [[A-star Search|A* Search]]，作用于 $m$ 维谓词空间
- **硬件**: 论文未明确
...
- [[A-star Search|A* Search]] / [[BFS]]: 测试时规划器
